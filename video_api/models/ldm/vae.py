...
class Upsample(nn.Upsample):
    "Fix bfloat16 support for NN interpolation"
    def forward(self, x):
        return super().forward(x.float()).type_as(x)
    
class Resample_Up(nn.Module):
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = attention_xformers_vae(q, k, v)
    ...
    def __init__(self,
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            ...
            if i != len(dim_mult) - 1:
                ...
                downsamples.append(Resample_Down(out_dim, mode=mode))
                scale /= 2.0
    ...
    def __init__(self,
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            ...
            if i != len(dim_mult) - 1:
                ...
                upsamples.append(Resample_Up(out_dim, mode=mode))
                scale *= 2.0
...
def encode_crop_pixels(pixels, downscale_ratio=8):
